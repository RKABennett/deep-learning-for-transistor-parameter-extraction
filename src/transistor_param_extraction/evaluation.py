...
This module contains functions for testing forward and inverse neural networks,
generating plots, and calculating performance metrics.
"""
...
from .scaling import unscale_vector
...
def plot_forward_comparison(X_test, test_predictions, errors, plot_folder_name, V):
    """
    Create detailed comparison plots for forward model predictions.
    """
    ...
    for i in range(np.shape(X_test)[0]):
        ...
        # Save data files
        data_files = [
            ("R2_{:.6f}_data_actual_Vds=0.1_linear.dat", [V, X_test[i, :, 0]]),
            ("R2_{:.6f}_data_pred_Vds=0.1_linear.dat", [V, test_predictions[i, :, 0]]),
            ("R2_{:.6f}_data_actual_Vds=0.1_log.dat", [V, X_test[i, :, 1]]),
            ("R2_{:.6f}_data_pred_Vds=0.1_log.dat", [V, test_predictions[i, :, 1]]),
            ("R2_{:.6f}_data_actual_Vds=1.0_linear.dat", [V, X_test[i, :, 2]]),
            ("R2_{:.6f}_data_pred_Vds=1.0_linear.dat", [V, test_predictions[i, :, 2]]),
            ("R2_{:.6f}_data_actual_Vds=1.0_log.dat", [V, X_test[i, :, 3]]),
            ("R2_{:.6f}_data_pred_Vds=1.0_log.dat", [V, test_predictions[i, :, 3]]),
        ]

        for filename_template, data in data_files:
            filename = filename_template.format(R2)
            np.savetxt(
                dir_path + "/" + plot_folder_name + "/" + filename, np.array(data)
            )
