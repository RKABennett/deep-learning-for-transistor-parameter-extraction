...
def train_inverse_NN(
    ...
    print("TRAINING COMPLETE.".format(starting_val_loss, np.min(val_loss_history)))
...
def test_model_inverse_params(
    ...
    for i in range(np.shape(Y_test)[1]):
        ...
        fig.suptitle(
            """
                     ...
                     Stdev of error = {:.4f} \n
                     """.format(
                np.median(abs_error), np.mean(abs_error), np.std(error)
            )
        )

        plt.tight_layout()
        plt.savefig(
            dir_path + "/{}_error_plot_idx={}.png".format(model_name_inverse, i)
        )
        plt.close()
    ...
    for i in range(10):
        ...
        donors_actual, acceptors_actual = calc_profile(
            E, Dpeak_actual, Dmid_actual, Dstd_actual, Apeak_actual, Astd_actual
        )

        donors_pred, acceptors_pred = calc_profile(
            E, Dpeak_pred, Dmid_pred, Dstd_pred, Apeak_pred, Astd_pred
        )
        ...
        nch_actual = calc_nch_vs_Ef(
            E, E, DOS_actual_profile, donors_actual, acceptors_actual
        )
        nch_pred = calc_nch_vs_Ef(E, E, DOS_pred_profile, donors_pred, acceptors_pred)

        plt.semilogy(E, nch_actual, color="k", ls="-", marker="None")

        plt.semilogy(E, nch_pred, color="r", ls="--", marker="None")
...
def test_model_inverse_current(
    ...
    for i in range(np.shape(X_test)[0]):
        ...
        for j in range(NNv.num_IdVg * 2):
            if deriv_error:
                error += error_metric(
                    np.gradient(X_test[i, :, j]), np.gradient(X_pred[i, :, j])
                )
            else:
    ...
    ax.hist(errors, density=False, edgecolor="k", linewidth=0.25, color="#1048a2")
...
def plot_forward_comparison(X_test, test_predictions, errors, plot_folder_name, V):
    ...
    for i in range(np.shape(X_test)[0]):
        ...
        ax1.plot(
            V[start:stop:skip],
            10**6 * X_test[i, :, 0][start:stop:skip],
            color="k",
            marker="o",
            ls="None",
        )
        ...
        ax1a.semilogy(
            V[start:stop:skip],
            10**6 * np.power(10, X_test[i, :, 1])[start:stop:skip],
            color="k",
            marker="o",
            ls="None",
        )
        ...
        ax2.plot(
            V[start:stop:skip],
            10**6 * X_test[i, :, 2][start:stop:skip],
            color="k",
            marker="o",
            ls="None",
        )
        ...
        ax2a.semilogy(
            V[start:stop:skip],
            10**6 * np.power(10, X_test[i, :, 3][start:stop:skip]),
            color="k",
            marker="o",
            ls="None",
        )
        ...
        np.savetxt(
            ...
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_actual_Vds=0.1_linear.dat".format(R2),
            np.array([V, X_test[i, :, 0]]),
        )
        np.savetxt(
            dir_path
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_pred_Vds=0.1_linear.dat".format(R2),
            np.array([V, test_predictions[i, :, 0]]),
        )
        ...
        np.savetxt(
            ...
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_actual_Vds=0.1_log.dat".format(R2),
            np.array([V, X_test[i, :, 1]]),
        )
        np.savetxt(
            dir_path
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_pred_Vds=0.1_log.dat".format(R2),
            np.array([V, test_predictions[i, :, 1]]),
        )
        ...
        np.savetxt(
            ...
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_actual_Vds=1.0_linear.dat".format(R2),
            np.array([V, X_test[i, :, 2]]),
        )
        np.savetxt(
            dir_path
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_pred_Vds=1.0_linear.dat".format(R2),
            np.array([V, test_predictions[i, :, 2]]),
        )
        ...
        np.savetxt(
            ...
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_actual_Vds=1.0_log.dat".format(R2),
            np.array([V, X_test[i, :, 3]]),
        )
        np.savetxt(
            dir_path
            + "/"
            + plot_folder_name
            + "/R2_{:.6f}_data_pred_Vds=1.0_log.dat".format(R2),
            np.array([V, test_predictions[i, :, 3]]),
        )

        plt.tight_layout()
        plt.savefig(
            dir_path + "/" + plot_folder_name + "/R2_{:.6f}plot_{}.png".format(R2, i)
        )
        plt.close()
