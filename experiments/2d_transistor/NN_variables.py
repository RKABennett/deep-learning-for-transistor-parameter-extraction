...
num_params = 8  # number of fitting parameters to extract
...
def build_model_forward(num_params, num_IdVg):
    ...
    NN_forward = GRU(
        512, return_sequences=True, activation="tanh", recurrent_activation="sigmoid"
    )(NN_forward)
    NN_forward = LayerNormalization()(NN_forward)
    output_layer_forward = Dense(num_IdVg * 2, activation="tanh")(NN_forward)
    model_forward = Model(inputs=input_layer_forward, outputs=output_layer_forward)
...
def build_model_inverse_smaller(num_points, num_IdVg, num_feats, num_params):
    ...
    output_layer_inverse = Dense(
        num_params + num_points * num_IdVg * 2, activation="tanh"
    )(NN)
...
def build_model_inverse_GRU():
    ...
    NN_inverse = GRU(
        256, return_sequences=True, activation="tanh", recurrent_activation="sigmoid"
    )(input_layer_inverse)

    output_layer_inverse = GRU(
        8, return_sequences=False, activation="tanh", recurrent_activation="sigmoid"
    )(NN_inverse)
...
def build_model_inverse(num_points, num_curves, num_output):
    ...
    output_layer_inverse = Dense(
        num_output + num_points * num_curves, activation="tanh"
    )(NN)
    model_inverse = Model(inputs=input_layer_inverse, outputs=output_layer_inverse)
