...
data_pred = np.loadtxt(
    dir_path
    + "/fits_inverse/NN_inverse_pred/error={:.12f}_pred.dat".format(target_error)
)
data_actual = np.loadtxt(
    dir_path
    + "/fits_inverse/NN_inverse_actual/error={:.12f}_actual.dat".format(target_error)
)
...
plt.savefig(
    dir_path
    + "/reverse_engineered_fit_quantile={}_R2={}.png".format(
        quantile, float(target_error)
    )
)
plt.close()
