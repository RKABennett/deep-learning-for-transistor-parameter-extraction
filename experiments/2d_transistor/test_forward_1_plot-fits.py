...
data_pred = np.loadtxt(
    dir_path
    + "/fits_forward/NN_forward_pred/error={:.12f}_pred.dat".format(target_error)
)
data_actual = np.loadtxt(
    dir_path
    + "/fits_forward/NN_forward_actual/error={:.12f}_actual.dat".format(target_error)
)
...
plt.savefig(
    dir_path
    + "/NN_forward_test_quantile={}_R2=.png".format(quantile, float(target_error)),
    transparent=False,
)
plt.close()
